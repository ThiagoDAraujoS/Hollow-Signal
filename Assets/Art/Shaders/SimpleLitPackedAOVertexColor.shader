Shader "HollowSignal/Environment/SimpleLitPackedAOVertexColor"
{
    Properties
    {
        ...
        [Header(Vertex Color Overlay)]
        [Toggle(_ENABLE_VERTEX_COLOR)] _EnableVertexColor ("Enable Vertex Color Overlay", Float) = 1.0
        _VertexColorStrength ("Vertex Color Strength", Range(0.0, 1.0)) = 1.0
        [KeywordEnum(Overlay, Multiply, SoftLight, Tint)] _VertexBlendMode ("Blend Mode", Float) = 0
        [Toggle] _VertexColorUseAlpha ("Use Vertex Alpha as Mask", Float) = 0.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _ENABLE_VERTEX_COLOR
            #pragma shader_feature_local _VERTEXBLENDMODE_OVERLAY _VERTEXBLENDMODE_MULTIPLY _VERTEXBLENDMODE_SOFTLIGHT _VERTEXBLENDMODE_TINT
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // Combine diffuse & ambient into final lit pixel
                half3 finalColor = creviceColor * (ambient + directLighting + additionalLighting);

                // 5. Vertex Color Overlay into Final Pixel
                #if defined(_ENABLE_VERTEX_COLOR)
                    half3 vColor = input.color.rgb;
                    half3 overlaid;
                    #if defined(_VERTEXBLENDMODE_OVERLAY)
                        // Standard Photoshop / Blender Overlay:
                        // Multiplies darks (< 0.5) and screens lights (>= 0.5)
                        overlaid = (finalColor < 0.5) ? (2.0 * finalColor * vColor) : (1.0 - 2.0 * (1.0 - finalColor) * (1.0 - vColor));
                    #elif defined(_VERTEXBLENDMODE_MULTIPLY)
                        overlaid = finalColor * vColor;
                    #elif defined(_VERTEXBLENDMODE_SOFTLIGHT)
                        overlaid = (1.0 - 2.0 * vColor) * finalColor * finalColor + 2.0 * vColor * finalColor;
                    #elif defined(_VERTEXBLENDMODE_TINT)
                        overlaid = lerp(finalColor, vColor, 0.5);
                    #else
                        overlaid = (finalColor < 0.5) ? (2.0 * finalColor * vColor) : (1.0 - 2.0 * (1.0 - finalColor) * (1.0 - vColor));
                    #endif

                    half vAlpha = lerp(1.0, input.color.a, _VertexColorUseAlpha);
                    finalColor = lerp(finalColor, overlaid, saturate(_VertexColorStrength * vAlpha));
                #endif
