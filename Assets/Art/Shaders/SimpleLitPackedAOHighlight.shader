Shader "HollowSignal/Interactable/SimpleLitPackedAOHighlight"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                half   _HighlightEdgeStrength;
                half   _HighlightEdgePower;
                half   _HighlightRimStrength;
                half   _HighlightRimPower;
                half   _HighlightPulseSpeed;
                half   _HighlightPulseDepth;
            ...
            // Computes Blender-style Ridge, Cavity, and raw Curvature
                ...
                out half outCurvature)
            {
                ...
                outCurvature = screenCurv + derivativeCurv;

                // Cavity (Valley / Inward Crevice)
                half valley = saturate(outCurvature * _CavityStrength);
                valley = pow(valley, _CavityPower);
                outCavityFactor = 1.0 - valley;

                // Ridge (Convex Peak / Outer Edge Highlight)
                half ridge = saturate(-outCurvature * _RidgeStrength);
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                half cavityFactor = 1.0;
                half3 ridgeTint = half3(1.0, 1.0, 1.0);
                half curvature = 0.0;

                // 3. Blender-style Ridge & Cavity Detection
                #if defined(_ENABLE_RIDGE_CAVITY)
                    EvaluateRidgeAndCavity(screenUV, input.positionWS, normalWS, input.positionCS.z, cavityFactor, ridgeTint, curvature);
                ...
                // 5. Interactive Highlight (Dramatic Mechanical Ridge + Silhouette Rim Glow)
                if (_HighlightAmount > 0.001)
                {
                    half pulse = 1.0;
                    if (_HighlightPulseSpeed > 0.01)
                    {
                        half wave = sin(_Time.y * _HighlightPulseSpeed) * 0.5 + 0.5;
                        pulse = lerp(1.0, wave, _HighlightPulseDepth);
                    }
                    half effectiveAmount = _HighlightAmount * pulse;

                    // A. Mechanical Edge / Ridge Highlight
                    #if !defined(_ENABLE_RIDGE_CAVITY)
                        EvaluateRidgeAndCavity(screenUV, input.positionWS, normalWS, input.positionCS.z, cavityFactor, ridgeTint, curvature);
                    #endif
                    half highlightRidge = saturate(-curvature * _HighlightEdgeStrength);
                    highlightRidge = pow(highlightRidge, _HighlightEdgePower);
                    half3 dramaticEdge = _HighlightEdgeColor.rgb * (highlightRidge * effectiveAmount);

                    // B. Outer Silhouette Fresnel Rim Glow
                    float3 viewDirWS = normalize(GetCameraPositionWS() - input.positionWS);
                    half NdotV = saturate(dot(normalWS, viewDirWS));
                    half rim = pow(1.0 - NdotV, _HighlightRimPower) * _HighlightRimStrength;
                    half3 dramaticRim = _HighlightRimColor.rgb * (rim * effectiveAmount);

                    // Emissive addition into final color
                    finalColor += (dramaticEdge + dramaticRim);
                }
