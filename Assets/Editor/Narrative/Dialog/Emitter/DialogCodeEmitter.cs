...
namespace Editor.Dialog.Emitter{
    ...
    public static class DialogCodeEmitter{
        ...
        /// Emits the C# class for the given AST, writes it to disk, and returns the path of the written file.
        public static string Emit(DialogScriptAst ast, string targetDirectory = null){
            ...
            // Collect distinct event hooks from all choices in all knots
            HashSet<string> distinctEvents = new();
            foreach (DialogKnotDef knot in ast.knots){
                foreach (DialogChoiceDef choice in knot.choices){
                    if (!string.IsNullOrEmpty(choice.eventName))
                        distinctEvents.Add(choice.eventName);
                }
            }

            if (distinctEvents.Count > 0){
                sb.AppendLine("        [Header(\"Dialogue Choice Events\")]");
                foreach (string evt in distinctEvents)
                    sb.AppendLine($"        public UnityEvent {SanitizeIdentifier(evt)} = new();");
                sb.AppendLine();
            }
        ...
        private static void EmitKnotMethod(StringBuilder sb, DialogKnotDef knot, string fileSlug, DialogScriptAst ast){
            ...
            if (knot.choices.Count > 0){
                ...
                for (int i = 0; i < knot.choices.Count; i++){
                    ...
                    string oneShotVal = choice.isOneShot ? "true" : "false";
                    ...
                    sb.AppendLine($"                        isOneShot = {oneShotVal},");
                    ...
                    if (!string.IsNullOrEmpty(choice.eventName)){
                        string sanitizedEvent = SanitizeIdentifier(choice.eventName);
                        sb.AppendLine($"                        onSelect = () => {sanitizedEvent}?.Invoke(),");
                    }
