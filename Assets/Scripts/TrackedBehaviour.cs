...
public abstract class TrackedBehaviour : MonoBehaviour, IBoundState{
    ...
    public virtual void OnSaveState(Partition state){
    ...
    public virtual void OnLoadState(Partition state){
        if (state == null) return;
        foreach (ITracked tracked in _trackedProperties)
            tracked.Load(state);
    }
...
public interface ITracked {
    string Key { get; }
    void Save(Partition state);
    void Load(Partition state);
}
